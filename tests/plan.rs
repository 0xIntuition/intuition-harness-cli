...
fn plan_help_lists_non_interactive_inputs() {
    cli()
        ...
        .stdout(predicate::str::contains("--questions <QUESTIONS>"))
...
#[cfg(unix)]
#[test]
fn plan_fast_no_interactive_skips_questions_and_addendum_but_uses_backlog_templates()
-> Result<(), Box<dyn Error>> {
    let temp = tempdir()?;
...
printf '%s' '{"summary":"Create one fast ticket.","issues":[{"title":"Fast plan ticket","description":"Create backlog artifacts through the shared template path.","acceptance_criteria":["fast mode skips follow-up Q&A in non-interactive runs","fast mode still writes the standard backlog files"],"priority":2}]}'
"#,
    )?;
    ...
    server.mock(|when, then| {
        when.method(POST)
            ...
            .body_includes("\"title\":\"Fast plan ticket\"")
            ...
            "data": {
                "issueCreate": {
                    ...
                    "issue": {
                        "id": "issue-91",
                        "identifier": "MET-91",
                        "title": "Fast plan ticket",
                        "description": "Create backlog artifacts through the shared template path.",
...
#[cfg(unix)]
#[test]
fn plan_fast_review_is_approve_or_reject_only() -> Result<(), Box<dyn Error>> {
    let temp = tempdir()?;
    let repo_root = temp.path().join("repo");
    let config_path = temp.path().join("metastack.toml");
    let bin_dir = temp.path().join("bin");
...
printf '%s' "$count" > "$count_file"
cat > "$TEST_OUTPUT_DIR/payload-$count.txt"
printf '%s' '{"summary":"Review a fast draft.","issues":[{"title":"Fast review ticket","description":"Show approve-or-reject only review controls.","acceptance_criteria":["review stays approve-or-reject only"],"priority":2}]}'
"#,
    )?;
    let mut permissions = fs::metadata(&stub_path)?.permissions();
    permissions.set_mode(0o755);
    ...
    let output = ProcessCommand::new("expect")
        ...
        .arg(format!(
            r#"
set timeout 10
spawn -noecho {meta_bin} plan --root {repo_root} --agent stub --fast --questions 0
expect {{
  -re "Planning Request" {{}}
}}
send -- "Render the fast planning review\r"
expect {{
  -re "Anything Else To Add\\?" {{}}
}}
send -- "\r"
expect {{
  -re "Generating fast draft" {{ exp_continue }}
  -re "Review Action" {{}}
}}
expect {{
  -re "Fast review is approve-or-reject only\\." {{}}
}}
send -- "r"
expect eof
"#,
    ...
    assert!(
        ...
        "interactive fast plan failed: stdout=`{}` stderr=`{}`",
  ...
  printf '%s' '{{\"summary\":\"Split the work into command wiring and dashboard flow.\",\"issues\":[{{\"title\":\"Add the {cmd} plan command\",\"description\":\"Introduce the top-level command and the deterministic non-interactive planning path.\",\"acceptance_criteria\":[\"`{cmd} plan --help` works\",\"Non-interactive planning can create backlog issues\"],\"priority\":2}},{{\"title\":\"Build the planning dashboard\",\"description\":\"Capture the request, follow-up answers, and ticket review in ratatui.\",\"acceptance_criteria\":[\"TTY planning runs show request, questions, and review states\",\"The dashboard confirms multi-issue creation before writing to Linear\"],\"priority\":3}}]}}'\n\
    ...
    let create_command_mock = server.mock(|when, then| {
        when.method(POST)
            .path("/graphql")
            .body_includes("mutation CreateIssue")
            .body_includes(format!("\"title\":\"Add the {} plan command\"", branding::COMMAND_NAME))
            .body_includes("\"projectId\":\"project-1\"")
            .body_includes("\"stateId\":\"state-backlog\"")
            .body_includes("\"labelIds\":[\"label-plan\"]");
        then.status(200).json_body(json!({
            "data": {
                "issueCreate": {
                    ...
                    "issue": {
                        "id": "issue-41",
                        "identifier": "MET-41",
                        "title": format!("Add the {} plan command", branding::COMMAND_NAME),
    ...
    let expected_title = format!("Add the {} plan command", branding::COMMAND_NAME);
    assert_plan_created_issues(
        &assert,
        &[
            ("MET-41", &expected_title, "Backlog", "MetaStack CLI", "MET"),
            (
                "MET-42",
    ...
    assert!(first_readme.contains(&format!("Add the {} plan command", branding::COMMAND_NAME)));
    ...
    assert!(first_proposed_prs.contains(&format!(
        "add-the-{}-plan-command-01",
        branding::COMMAND_NAME
    )));
...
if [ \"$count\" -eq 1 ]; then\n  printf '%s' '{{\"questions\":[]}}'\n\
    ...
    let create_issue_mock = server.mock(|when, then| {
        when.method(POST)
            .path("/graphql")
            .body_includes("mutation CreateIssue")
            .body_includes(format!("\"title\":\"Fix the {} plan command\"", branding::COMMAND_NAME))
            .body_includes("\"projectId\":\"project-1\"")
            .body_includes("\"stateId\":\"state-backlog\"")
            .body_includes("\"labelIds\":[\"label-plan\"]");
        then.status(200).json_body(json!({
            "data": {
                "issueCreate": {
                    ...
                    "issue": {
                        "id": "issue-42",
                        "identifier": "MET-42",
                        "title": format!("Fix the {} plan command", branding::COMMAND_NAME),
    ...
    let fix_title = format!("Fix the {} plan command", branding::COMMAND_NAME);
    assert_plan_created_issues(
        &assert,
        &[("MET-42", &fix_title, "Backlog", "MetaStack CLI", "MET")],
    );
