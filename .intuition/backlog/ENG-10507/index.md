...
- [x] Added the `agents build` CLI command, dispatch wiring, and `agents.build` route resolution.
- [x] Implemented workspace resolution, git-repository validation, interactive looping, queued prompt handling, and provider-aware continuation behavior.
- [x] Added command-path and help-surface coverage for `agents build`, plus README command documentation.
- [x] Preserved the managed listen-progress block when reusing existing backlog packet descriptions during `listen` workspace refreshes.

### Remaining

- [x] No remaining implementation work captured in the local progress block.

### Validation

- [x] `cargo test --test build -- --nocapture`
- [x] `cargo test --test commands -- --nocapture`
- [x] `cargo test --test config -- --nocapture`
- [x] `cargo test render_listen_backlog_description_preserves_existing_body_and_appends_progress_block --lib`
- [x] `cargo test --test listen listen_once_refreshes_existing_workspace_clone_and_reuses_backlog_and_workpad_comment -- --nocapture`
- [x] `cargo clippy --all-targets --all-features -- -D warnings`
- [x] `cargo run -- agents build --help`
- [x] `cargo run -- agents build --dir <tmp-workspace> "test" --root <tmp-repo> --no-interactive`
