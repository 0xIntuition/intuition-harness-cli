...
`intu workflows` loads repo-local playbooks from this directory in addition to the built-in workflow set.
...
- `name`: unique workflow identifier used by `intu workflows explain|run`
- `summary`: one-line description shown by `intu workflows list`
...
- `{effective_instructions}` for only the optional repo-scoped instructions file from `.intuition/meta.json`
