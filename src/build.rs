...
fn resolve_build_session(
    ...
    Ok(BuildSession {
        ...
        requested_agent: args.agent.clone(),
        requested_model: args.model.clone(),
        requested_reasoning: args.reasoning.clone(),
...
fn run_build_turn(
    ...
    let run_args = RunAgentArgs {
        ...
        route_key: Some(AGENT_ROUTE_AGENTS_BUILD.to_string()),
        agent: session.requested_agent.clone(),
        prompt,
        instructions: None,
        model: session.requested_model.clone(),
        reasoning: session.requested_reasoning.clone(),
        transport: None,
        attachments: Vec::new(),
    };
